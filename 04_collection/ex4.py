...
p1 = (1, 2, 3)
p2 = (10, 20, 30)
result = tuple((p1[x] + p2[x]) for x in range(0, 3))
...
print(f"max 점수:{max(scores)}, min 점수:{min(scores)}")
result = sorted(scores)
print(f"max 점수:{result[-1]}, min 점수:{result[0]}")
...
total = sum(i[1] * i[2] for i in stocks)
...
total = sum(x*y for x,y in zip(stocks[1],stocks[2]))
print(f"{total}원")
